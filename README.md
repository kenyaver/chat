...
#### 2.2.1. Сервер получает первую команду.
        ...
        2.2.1.3. Отправляет все оффлайн-команды с type=4 (если есть).
####    2.2.2. Сервер ждет событий по сокетам (клиента или таймера).
#####        2.2.2.1. Сервер получает команду.
######            2.2.2.1.1.1. type = 0:
                2.2.2.1.1.1.1. Команда пересылается получателю.
                2.2.2.1.1.1.2. Включается таймер ожидания ответа.
######            2.2.2.1.1.2. type = 1:
                2.2.2.1.1.2.1. Таймер для команды с таким messageID для этого клиента сбрасывается.
                2.2.2.1.1.2.2. Если клиент подключен к серверу, то команда отправляется клиенту.
######            2.2.2.1.1.3. type = 2:
                2.2.2.1.1.3.1. Сервер сохраняет нового получателя и будет отправлять все команды ему до следующего type=2.
                2.2.2.1.1.3.2. Выполняется шаг 2.2.3.1.
######            2.2.2.1.1.4. type = 3:
                2.2.2.1.1.4.1. Команда пересылается партнеру.
                2.2.2.1.1.4.2. Клиент отсоединяется от сервера.
                2.2.2.1.1.4.3. Буффер неподтвержденных команд очищается.
                2.2.2.1.1.4.4. Поток завершает работу.
#####        2.2.2.2. Если был включен таймер и завершился без ответа клиента, то все обрабатываемые команды сохраняются в специальном файле.
            2.2.2.2.1. Статус партнера меняется на оффлайн.
####    2.2.3. При получении сигнала завершения работы сервера, сервер записывает в файл offline/{DST_username}.txt все обрабатываемые команды из буффера команд.
####    2.2.4. Повторяется шаг 2.2.2.


## Клиент:

### 3.1. Начало работы приложения
    Запуск с указанием адреса и порта сервера, username пользователя и партнера.

### 3.2. Подключение к серверу
    Подключение к серверу по протоколу TCP.

### 3.3. Подготовка к обмену командами
    Запуск потоков на отправку и принятие команд.
...
### 3.6. Завершение работы приложения
    Для завершения приложения, отправляется символ '~' в любом месте сообщения.
